...
import robust_disentangling.nn as nn
from robust_disentangling.triplet import triplet_wrapper
...
class TrainModel(object):
    ...
    def define_graph(self):
        ...
        for view_idx in range(2):
            v = tf.placeholder(
                ...
                shape = (
                    ...
                    self.config["spatial_size"],
                    3), name = "input_view_{}".format(view_idx))
            self.views.append(v)
        ...
        if self.config.get("train_encoder_1", False):
            e1 = nn.make_model("encoder_1", encoder_model, out_size = z1_size)
            self.e1_name = "my_triplet_is_the_best_triplet" # this is just used
                                                            # to ignore variables
        else:
        ...
        if self.pretty:
            pretty_discriminator = nn.make_model("pretty_discriminator", pretty_discriminator_model)
        ...
        self._cross_generated = dd(z_00_distribution.sample(), tf.stop_gradient(z_1_independent))
        if self.pretty:
            # discriminator for improved perceptual reconstruction
            ...
            self.logit_pretty_fake, self.feat_pretty_fake = pretty_discriminator(self.fake_input, tf.stop_gradient(z_1_independent))

        # mi gradients
        self.logit_joint0 = mi0_discriminator((z_00_distribution.sample(), z_11))
        self.logit_marginal0 = mi0_discriminator((z_00_distribution.sample(), z_1_independent))

        self.logit_joint1 = mi1_discriminator((z_00_distribution.sample(), z_11))
        self.logit_marginal1 = mi1_discriminator((z_00_distribution.sample(), z_1_independent))

        # mi estimation
        self.mi_logit_joint = mi_estimator((z_00_distribution.sample(), z_11))
        self.mi_logit_marginal = mi_estimator((z_00_distribution.sample(), z_1_independent))
...
class Trainer(TFBaseTrainer):
    def lazy_init(self, checkpoint_path = None):
    ...
    def initialize(self, checkpoint_path = None):
        return_ = self.lazy_init(checkpoint_path)
        if not self.config.get("train_encoder_1", False):
    ...
    def make_loss_ops(self):
        ...
        with tf.variable_scope('VGG19__noinit__'):
            ...
            self.vgg19 = vgg19 = deeploss.VGG19Features(self.session,
                    default_gram = gram_weight, original_scale = True)
        dim = np.prod(self.model.inputs["view0"].shape.as_list()[1:])
        auto_rec_loss = 1e-3*0.5*dim*vgg19.make_loss_op(self.model.inputs["view0"], self.model._generated)
        vis_rec_loss = tf.reduce_mean(
                    tf.square(
                        self.model.inputs["view0"] -
                        self.model._visualize))
        app_rec_loss = tf.reduce_mean(
                    tf.square(
                        self.model.inputs["view1"] -
                        self.model._app_visualize))
        ...
        if self.config.get("train_encoder_1", False):
            ...
            if self.config.get("maxinfo_encoder_1", 0.0) > 0.0:
                weight = tf.cast(self.config["maxinfo_encoder_1"], tf.float32)
                losses["encoder_1"] = app_rec_loss
        ...
        # Parameters
        # epsilon
        if "MI" in self.config:
            MI_TARGET = self.config["MI"].get("mi_target", 0.125)
        else:
            MI_TARGET = self.config.get("MI_TARGET", 0.125)
        ...
        independent_mim_constraint = logit_constraint(self.model.logit_joint1, real = False)

        # level of overpowering
        avg_mim_constraint = tf.maximum(0.0, make_ema(0.0, mim_constraint, self.update_ops))
        avg_independent_mim_constraint = tf.maximum(0.0, make_ema(0.0, independent_mim_constraint, self.update_ops))
        ...
        loo = tf.clip_by_value(loo, 0.0, 1e1)
        ...
        if adversarial_regularization:
            ...
            loa_gain = mim_constraint - MI_TARGET
            ...
            if LOA_ADAPTIVE:
                ...
                adversarial_active = tf.stop_gradient(tf.to_float(loa_lr*loa_gain >= -loa))
                adversarial_weighted_loss = adversarial_active*(
                        loa*loa_gain +
                        loa_lr/2.0*tf.square(loa_gain))
            else:
        ...
        if variational_regularization:
            ...
            lor_gain = loo - LOR_BIAS
            new_lor = tf.clip_by_value(lor+LOR_LR*lor_gain, 0.0, 1e3)
            if LOR_ADAPTIVE:
                update_lor = tf.assign(lor, new_lor)
                self.update_ops.append(update_lor)

            bottleneck_weighted_loss = lor*bottleneck_loss
        ...
        if self.model.pretty:
            ...
            pretty_objective = 2.0*pw*loss_dec_pretty # backward comp for comparison
